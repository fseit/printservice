...
class admin_plugin_printservice_lecturematerials extends DokuWiki_Admin_Plugin {
	...
	public function html() {
		...
		ptln ( '<h1>' . $this->getLang ( 'menu_lecturematerials' ) . '</h1>' );
		$dbhelper =& plugin_load('helper','printservice_database');
		$dbhelper->dbConnect ();
		if (! $semesters = $dbhelper->fetchAllSemesters ()) {
			...
			$lecturematerials = $dbhelper->fetchLectureMaterials ();
			$unknown = $dbhelper->fetchUnknownLectureMaterials ();
			...
			//editunknown
			$form = new Doku_Form ( array ('id' => 'editunknownlecturematerials', 'method' => 'POST' ) );
			$form->startFieldSet ( $this->getLang ('field_editunknownlecturematerials') );
			$form->addHidden ( 'page', 'printservice_lecturematerials' );
			...
			while ( $row = $unknown->fetchRow () ) {
				...
				$form->addElement ( '</td><td>'.$lecturers[$row ['lecturer']].'</td>' );
				...
				$form->addElement ( '<td><select size="1" name="edit_document[]">' );
				$form->addElement ( '<option value="0">---</option>' );
				foreach ( $documents as $key => $value ) {
			...
			//edit
			$form = new Doku_Form ( array ('id' => 'editlecturematerials', 'method' => 'POST' ) );
			$form->startFieldSet ( $this->getLang ('field_editlecturematerials') );
			$form->addHidden ( 'page', 'printservice_lecturematerials' );
			$form->addElement ( '<table><tr><th>'.$this->getLang ('tbl_id').'</th><th>'.$this->getLang ('tbl_lecture').'</th><th>'.$this->getLang ('tbl_lecturer').'</th><th>'.$this->getLang ('tbl_doc').'</th><th>'.$this->getLang ('tbl_delete').'</th></tr>' );
			while ( $row = $lecturematerials->fetchRow () ) {
				$form->addElement ( '<tr><td>' . $row ['id'] );
				$form->addElement ( '<input type="hidden" value="' . $row ['id'] . '" name="edit_id[]">' );
				$form->addElement ( '</td><td>' );
				$form->addElement ( hsc ( $lectures [$row ['lecture']] ) );
